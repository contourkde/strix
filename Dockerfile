...
RUN mkdir -p /home/pentester/configs \
             ...
             /app/certs \
             /workspace && \
    chown -R pentester:pentester /app/certs /home/pentester/tools /workspace /app
...
# Clone Strix repository and configure
USER root
RUN git clone https://github.com/usestrix/strix.git /tmp/strix_repo && \
    cp /tmp/strix_repo/pyproject.toml /tmp/strix_repo/uv.lock ./ && \
    echo "# Sandbox Environment" > README.md && mkdir -p strix && touch strix/__init__.py && \
...
USER root
RUN cp /tmp/strix_repo/containers/docker-entrypoint.sh /usr/local/bin/docker-entrypoint.sh && \
    chmod +x /usr/local/bin/docker-entrypoint.sh && \
    rm -rf /tmp/strix_repo
...
# Keep the container running for headless execution
CMD ["sleep", "infinity"]
